...
Please be advised that the included code supports the training and analysis pipeline for the publicly available Mante 2013 dataset (accessible at https://www.ini.uzh.ch/en/research/groups/mante/data.html). The Sussillo 2015 data is not included as we do not have permission to redistribute it. The pipeline utilizes preprocessed neural data, where spikes are converted to rate data and each trial represents a condition average. For RNN training, synthetic data generated by Neurogym is used. 